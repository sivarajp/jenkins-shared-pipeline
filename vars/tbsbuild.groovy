                ...
                stage ('Docker build and push')   {
                    container ('pb') {
                        ...
                        script {
                            ...
                            sh "pb project target ${config.project}"
                            sh "kubectl config use-context tbs-default-siva-aws-poc --namespace ${config.namespace}"
                            withCredentials([file(credentialsId: "${config.project}-registry", variable: "registry")]) {
                                     sh "pb secrets registry apply -f $registry"
                            }
                            withCredentials([file(credentialsId: "${config.project}-repo", variable: "repo")]) {
